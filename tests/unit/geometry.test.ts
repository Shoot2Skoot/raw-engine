...
describe('Geometry utilities', () => {
  describe('pointInRect', () => {
    ...
    test('should detect point outside rectangle', () => {
      ...
      expect(Geometry.pointInRect({ x: 50, y: -1 }, rect)).toBe(false);
      expect(Geometry.pointInRect({ x: 101, y: 50 }, rect)).toBe(false);
      expect(Geometry.pointInRect({ x: 50, y: 101 }, rect)).toBe(false);
    });
  });
  ...
  describe('generateGridHotspots', () => {
    test('should generate correct number of hotspots', () => {
    ...
    test('should generate hotspots with correct IDs', () => {
      const hotspots = Geometry.generateGridHotspots(
        {
          type: 'grid',
          rows: 2,
    ...
    test('should generate hotspots with correct positions', () => {
      const hotspots = Geometry.generateGridHotspots(
        {
          type: 'grid',
          rows: 2,
      ...
      expect(hotspots[3].position).toEqual({ x: 160, y: 260 });
    });
  });
});
