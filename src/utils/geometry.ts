...
export class Geometry {
  ...
  static getHotspotCenter(hotspot: Hotspot): Point {
    switch (hotspot.shape) {
      case 'rect':
        return {
          x: hotspot.position.x + (hotspot.size?.width || 0) / 2,
          y: hotspot.position.y + (hotspot.size?.height || 0) / 2
        };
      case 'circle':
      case 'point':
      ...
      case 'polygon': {
        ...
        // Calculate centroid
        let sumX = 0, sumY = 0;
        for (const point of hotspot.points) {
          sumX += point.x;
          sumY += point.y;
        }
        return {
          x: sumX / hotspot.points.length,
          y: sumY / hotspot.points.length
        };
      }
    }
  }
