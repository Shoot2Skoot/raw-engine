...
import { Square, Hash, PaintBucket, Circle, Pencil, Type, Undo, Redo, Save, Trash2 } from 'lucide-react';
...
export const Toolbar: React.FC = () => {
  ...
  const handleSave = () => {
    ...
    Serialization.saveToLocalStorage('roll-and-write-save', state);
    alert('Game saved!');
  };

  const handleLoad = () => {
    const state = Serialization.loadFromLocalStorage('roll-and-write-save');
    if (state) {
  ...
  return (
    <div className="flex items-center justify-between p-4 bg-gray-100 rounded-lg border border-gray-300 mb-4">
      ...
      <div className="flex items-center gap-2">
        ...
        <button
          onClick={handleSave}
          className="px-4 py-2 rounded-lg bg-white hover:bg-gray-50 transition-colors flex items-center gap-2"
          title="Save"
        >
          <Save size={20} />
        ...
        <button
          onClick={handleLoad}
          className="px-4 py-2 rounded-lg bg-white hover:bg-gray-50 transition-colors text-sm font-medium"
          title="Load"
        >
          Load
        </button>
        ...
        <button
          onClick={handleClear}
          className="p-2 rounded-lg bg-white hover:bg-red-50 text-red-600 transition-colors"
          title="Clear All"
        >
          <Trash2 size={20} />
        </button>
