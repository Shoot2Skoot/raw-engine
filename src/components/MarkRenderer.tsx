...
export const MarkRenderer: React.FC<MarkRendererProps> = React.memo(
  ({ mark, hotspot, isHovered }) => {
    ...
    return (
      <g
        ...
        className={`transition-opacity animate-mark-appear ${isHovered ? 'opacity-75' : ''} ${
  ...
  (prev, next) => {
    return prev.mark.id === next.mark.id && prev.isHovered === next.isHovered;
  }
);
