...
export const MarkRenderer: React.FC<MarkRendererProps> = ({
  ...
  const getTransform = () => {
    ...
    if (shape === 'rect' && size) {
      return `translate(${position.x + size.width / 2}, ${
    ...
    switch (mark.type) {
      case 'checkbox':
        if (mark.value === 'checked') {
          return (
            <path
              d={`M ${-halfWidth * 0.5} 0 L ${-halfWidth * 0.2} ${
                halfHeight * 0.4
              } L ${halfWidth * 0.6} ${-halfHeight * 0.5}`}
              stroke="currentColor"
              strokeWidth="3"
              strokeLinecap="round"
              fill="none"
  ...
  return (
    <g
      ...
      className={`transition-opacity mark-appear ${
...
// Helper component for hotspot highlight on hover
export const HotspotHighlight: React.FC<{ hotspot: Hotspot }> = ({
  hotspot,
}) => {
...
export const HotspotOutline: React.FC<{ hotspot: Hotspot }> = ({ hotspot }) => {
  const renderOutline = () => {
    switch (hotspot.shape) {
      case 'rect':
        return (
