...
/**
 * Helper component for hotspot hover highlight
 */
...
/**
 * Helper component for debugging hotspot outlines
 */
...
export const SheetCanvas: React.FC = () => {
  ...
  const handlePointerDown = (e: React.PointerEvent<SVGSVGElement>) => {
    ...
    if (['number', 'fill', 'symbol', 'text'].includes(currentTool)) {
      ...
      // Toggle through states for checkbox and circle
  ...
  return (
    <div className="relative">
      <svg
        ...
        className="w-full h-auto border-2 border-gray-300 rounded-lg shadow-lg touch-none bg-white"
        ...
        {definition.regions.map(region => (
          <g key={region.id} style={{ zIndex: region.zIndex }}>
            {region.hotspots?.map(hotspot => (
              <g key={hotspot.id} data-hotspot-id={hotspot.id}>
