...
export const SheetCanvas: React.FC = () => {
  ...
  const handlePointerDown = (e: React.PointerEvent<SVGSVGElement>) => {
    ...
    const sheetPoint = CoordinateTransform.screenToSheet(
      screenPoint,
      svgRef.current
    );
    ...
    // Check if the tool can be placed on this hotspot
    if (!engine.canPlaceMark(hotspot)) {
      console.warn('Cannot place mark on this hotspot');
      return;
    }
    ...
    if (['number', 'fill', 'symbol', 'text'].includes(currentTool)) {
      ...
      // Direct mark for checkbox, circle, etc. (toggle behavior)
...
const HotspotHighlight: React.FC<{ hotspot: any }> = ({ hotspot }) => {
  const renderHighlight = () => {
    switch (hotspot.shape) {
      ...
      case 'polygon':
        const points = hotspot.points?.map((p: Point) => `${p.x},${p.y}`).join(' ');
        return (
          <polygon
            points={points}
            fill="blue"
...
const HotspotOutline: React.FC<{ hotspot: any }> = ({ hotspot }) => {
  const renderOutline = () => {
    switch (hotspot.shape) {
      case 'rect':
        return (
