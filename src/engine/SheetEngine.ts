...
export class SheetEngine {
      ...
      const regions = def.regions.map(region => {
        if (region.type === 'grid' && region.layout) {
          ...
          const hotspots = Geometry.generateGridHotspots(
            region.layout,
            region.hotspots?.[0]?.allowedMarkTypes || ['number', 'checkbox'] // Use first hotspot's allowed types or default
          );
  ...
  // Sync hotspot references with marks map
