...
export class SheetEngine {
  ...
  canPlaceMark(hotspot: Hotspot): boolean {
    ...
    // Check mark limit (for single-mark hotspots)
    if (hotspot.maxMarks === 1 && hotspot.currentMark) {
  ...
  addMark(hotspotId: string, value?: string | number): boolean {
    ...
    const markValue = value !== undefined ? value :
                      this.currentValue || this.getDefaultValue(this.currentTool);
  ...
  removeMark(hotspotId: string): boolean {
    ...
    // Find and clear hotspot reference
    for (const region of sheet.definition.regions) {
  ...
  undo(): boolean {
    ...
    if (result) {
      // Sync hotspot references after undo
      this.syncHotspotReferences();
    }
    return result;
  }
  ...
  redo(): boolean {
    ...
    if (result) {
      // Sync hotspot references after redo
      this.syncHotspotReferences();
    }
    return result;
  }
  ...
  private syncHotspotReferences(): void {
    // Sync all hotspot currentMark references with the marks map
    this.sheets.forEach(sheet => {
      sheet.definition.regions.forEach(region => {
        region.hotspots?.forEach(hotspot => {
          hotspot.currentMark = sheet.marks.get(hotspot.id);
        });
      });
    });
  }
