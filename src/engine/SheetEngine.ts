...
export class SheetEngine {
  ...
  // Find hotspot by id
  findHotspot(hotspotId: string): Hotspot | undefined {
    const sheet = this.getCurrentSheet();
  ...
  canPlaceMark(hotspot: Hotspot): boolean {
    ...
    // For now, we allow replacing marks (no maxMarks check)
  ...
  addMark(hotspotId: string, value?: string | number): boolean {
    ...
    let markValue: string | number | boolean = value ?? this.currentValue;
    ...
    if (this.currentTool === 'checkbox' && targetHotspot.currentMark?.type === 'checkbox') {
    ...
    } else if (this.currentTool === 'circle' && targetHotspot.currentMark?.type === 'circle') {
      ...
      const defaultValue = this.getDefaultValue(this.currentTool);
      markValue = markValue || defaultValue;
    }
